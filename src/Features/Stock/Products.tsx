...
export default function Products({}: Props) {
  ...
  useEffect(() => {
    DubenzeneFetcher.get("/clients").then(({ data }: any) => {
      setClients(data?.data);
    });
  }, []);

  const deleteClient = (id: any) => {
    DubenzeneFetcher.delete("/clients/" + id).then(({ data }: any) => {
      setClients(clients.filter((value: any) => value.id != id));
      setConfirmDeleteShow(false);
      setClientInDelete(null);
    });
  };
  ...
  const createClient = () => {
    ...
    DubenzeneFetcher.post("/clients", dataToSend).then(({ data }: any) => {
      let copy: any = [...clients];
                ...
                {clientsFilter.map((value: any, index: number) => (
                  <tr>
                    ...
                    <th className="underline cursor-pointer select-none font-[PoppinsBold] ">
                      <Link
                        to={"/dashboard/products-manager/products/" + value.id}
                        state={value}
                      >
                        {" "}
                        {value.name}{" "}
      ...
      >
        <div className="bg-zinc-50 px-6 pt-2 text-lg rounded-lg w-1/4">
          ...
          <div className="h-64 flex flex-col items-center p-8 text-center">
            ...
            <a
              onClick={() => deleteClient(clientInDelete?.id)}
