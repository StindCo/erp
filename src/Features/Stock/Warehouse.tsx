...
function Warehouse(props: Props) {
  ...
  useEffect(() => {
    setWarehouse(
      warehousesFounded.filter((value: any) => value?.idTech == id) ?? {}
    );
      ...
      <div className="w-1/4 bg-[#fefefe] shadow-lg h-full rounded-t-lg">
        <div className="p-8 text-center space-y-3">
          ...
          <div className="space-y-2">
            <p className="text-xl font-[PoppinsBold]">{warehouse?.name}</p>
            <p className="text-slate-800">{warehouse.phoneNumber}</p>
