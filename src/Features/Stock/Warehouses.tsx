...
export default function Warehouses({}: Props) {
  ...
  useEffect(() => {
    DubenzeneFetcher.get("/fournisseurs").then(({ data }: any) => {
      setFournisseurs(data?.data);
    });
  }, []);

  const deleteFournisseur = (id: any) => {
    DubenzeneFetcher.delete("/fournisseurs/" + id).then(({ data }: any) => {
      setFournisseurs(fournisseurs.filter((value: any) => value.id != id));
      setConfirmDeleteShow(false);
      setFournisseurInDelete(null);
    });
  };
  ...
  const createFournisseur = () => {
    ...
    DubenzeneFetcher.post("/fournisseurs", dataToSend).then(({ data }: any) => {
      let copy: any = [...fournisseurs];

      copy.push(data.data);
      setFournisseurs(copy);
                ...
                {fournisseursFilter.map((value: any, index: number) => (
                  <tr key={Math.random()}>
                    ...
                    <th className="underline cursor-pointer select-none font-[PoppinsBold] ">
                      <Link to={"/dashboard/products-manager/warehouses/" + value.id}>
                        {" "}
                        {value.name}{" "}
                      </Link>
                    </th>
      ...
      >
        <div className="bg-zinc-50 px-6 pt-2 text-lg rounded-lg w-1/4">
          ...
          <div className="h-64 flex flex-col items-center p-8 text-center">
            ...
            <a
              onClick={() => deleteFournisseur(fournisseurInDelete?.id)}
